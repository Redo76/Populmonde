...
function regionsData($idContinent){
    $db = PDO();
    $regionsDB = $db -> prepare("SELECT r.libelle_region as nom, SUM(p.population_pays) as pop, ROUND(AVG(p.taux_natalite_pays) , 2)as nat, ROUND(AVG(p.taux_mortalite_pays),2)  as mort, ROUND(AVG(p.esperance_vie_pays),2) as esp, ROUND(AVG(p.taux_mortalite_infantile_pays),2) as mortinf, ROUND(AVG(p.nombre_enfants_par_femme_pays),2) as enf, ROUND(AVG(p.taux_croissance_pays),2) as croiss, SUM(p.population_plus_65_pays) as 65p, SUM(p.superficie_km2) as sup FROM t_pays p INNER JOIN t_regions r ON p.region_id = r.id_region INNER JOIN t_continents c ON r.continent_id = c.id_continent WHERE c.id_continent = :id GROUP BY r.libelle_region
    UNION 
    SELECT c.libelle_continent as nom, SUM(p.population_pays) as pop, ROUND(AVG(p.taux_natalite_pays) , 2)as nat, ROUND(AVG(p.taux_mortalite_pays),2) as mort, ROUND(AVG(p.esperance_vie_pays),2) as esp, ROUND(AVG(p.taux_mortalite_infantile_pays),2) as mortinf, ROUND(AVG(p.nombre_enfants_par_femme_pays) ,2) as enf, ROUND(AVG(p.taux_croissance_pays) ,2) as croiss, SUM(p.population_plus_65_pays) as 65p, SUM(p.superficie_km2) as sup FROM t_pays p INNER JOIN t_continents c ON p.continent_id = c.id_continent WHERE c.id_continent = :id");
...
function paysData($idRegion){
    $db = PDO();
    $paysDB = $db -> prepare("SELECT p.libelle_pays as nom, SUM(p.population_pays) as pop, ROUND(AVG(p.taux_natalite_pays) , 2)as nat, ROUND(AVG(p.taux_mortalite_pays),2) as mort, ROUND(AVG(p.esperance_vie_pays),2) as esp, ROUND(AVG(p.taux_mortalite_infantile_pays),2) as mortinf, ROUND(AVG(p.nombre_enfants_par_femme_pays) ,2) as enf, ROUND(AVG(p.taux_croissance_pays) ,2) as croiss, SUM(p.population_plus_65_pays) as 65p, SUM(p.superficie_km2) as sup FROM t_pays p INNER JOIN t_regions r ON p.region_id = r.id_region WHERE r.id_region = :id GROUP BY p.libelle_pays
    UNION 
    SELECT r.libelle_region as nom, SUM(p.population_pays) as pop, ROUND(AVG(p.taux_natalite_pays) , 2)as nat, ROUND(AVG(p.taux_mortalite_pays),2) as mort, ROUND(AVG(p.esperance_vie_pays),2) as esp, ROUND(AVG(p.taux_mortalite_infantile_pays),2) as mortinf, ROUND(AVG(p.nombre_enfants_par_femme_pays) ,2) as enf, ROUND(AVG(p.taux_croissance_pays) ,2) as croiss, SUM(p.population_plus_65_pays) as 65p, SUM(p.superficie_km2) as sup FROM t_pays p INNER JOIN t_regions r ON p.region_id = r.id_region WHERE r.id_region = :id");
...
function ameriqueSept(){
    $db = PDO();
    $amerDB = $db -> prepare("SELECT p.libelle_pays as nom, SUM(p.population_pays) as pop, ROUND(p.taux_natalite_pays , 2) as nat, ROUND(p.taux_mortalite_pays,2) as mort, ROUND(p.esperance_vie_pays,2) as esp, ROUND(p.taux_mortalite_infantile_pays,2) as mortinf, ROUND(p.nombre_enfants_par_femme_pays, 2) as enf, ROUND(p.taux_croissance_pays, 2) as croiss, SUM(p.population_plus_65_pays) as 65p, SUM(p.superficie_km2) as sup FROM t_pays p WHERE p.continent_id = 3 GROUP BY p.libelle_pays
    UNION 
    SELECT c.libelle_continent as nom, SUM(p.population_pays) as pop, ROUND(AVG(p.taux_natalite_pays) , 2)as nat, ROUND(AVG(p.taux_mortalite_pays),2) as mort, ROUND(AVG(p.esperance_vie_pays),2) as esp, ROUND(AVG(p.taux_mortalite_infantile_pays),2) as mortinf, ROUND(AVG(p.nombre_enfants_par_femme_pays) ,2) as enf, ROUND(AVG(p.taux_croissance_pays) ,2) as croiss, SUM(p.population_plus_65_pays) as 65p, SUM(p.superficie_km2) as sup FROM t_pays p INNER JOIN t_continents c ON p.continent_id = c.id_continent WHERE c.id_continent = 3");
